...
contract HyperEX is PoSTokenStandard {
    ...
    uint8 internal constant entryFee_ = 12;

    /// @dev 15% dividends for token selling
    uint8 internal constant startExitFee_ = 15;

    /// @dev 15% dividends for token selling after step
    uint8 internal constant finalExitFee_ = 15;

    /// @dev Exit fee falls over period of 30 days
    uint256 internal constant exitFeeFallDuration_ = 1 days;

    /// @dev 10% masternode
    uint8 internal constant refferalFee_ = 10;
    ...
    function buy(
        address _referredBy
    ) public payable antiEarlyWhale notGasbag isControlled returns (uint256) {
     ...
     */
    function() external payable antiEarlyWhale notGasbag isControlled {
    ...
    function withdraw() public onlyStronghands {
        ...
        dividendPaidTotal[_customerAddress] = dividendPaidTotal[
            _customerAddress
        ].add(_dividends);
    ...
    function sell(uint256 _amountOfTokens) public onlyBagholders {
        ...
        uint256 _dividends = SafeMath.div(
            SafeMath.mul(_ethereum, exitFee()),
            100
        );
        ...
        tokenTrackerForDividends[_customerAddress] = tokenTrackerForDividends[
            _customerAddress
        ].sub(_tokens);

        // update dividends tracker
        uint256 _updatedPayouts = (profitPerShare_ *
            _tokens +
            (_taxedEthereum * magnitude));
    ...
    function transfer(
    ...
    ) public onlyBagholders returns (bool) {
        ...
        tokenTrackerForDividends[_toAddress] = tokenTrackerForDividends[
            _toAddress
        ].add(_amountOfTokens);
       ...
       */
        ...
        if (transferIns[_customerAddress].length > 0)
            delete transferIns[_customerAddress];
        uint64 _now = uint64(now);
        transferIns[_customerAddress].push(
            transferInStruct(
                uint128(tokenBalanceLedger_[_customerAddress]),
                _now
            )
        );
        transferIns[_toAddress].push(
            transferInStruct(uint128(_amountOfTokens), _now)
        );
    ...
    function dividendsOf(
        ...
        uint256 sharePercentage = (tokenTrackerForDividends[_customerAddress] *
            100 *
            1000000) / tokenSupply_;

        if (sharePercentage > 0) {
            uint256 _dividends = (((dividendPoolNEW * sharePercentage) / 100) /
                1000000); //dividing with 1000000 because sharePercentage was multiplied with it.
    ...
    function changeDividendsFee(uint256 _newDividendsFee) public {
        require(
            msg.sender == apex,
            "Apex only can make changes to dividendsFee"
        );
    ...
    function buyPrice() public view returns (uint256) {
        ...
        if (tokenSupply_ == 0) {
            ...
            uint256 _ethereum = tokensToEthereum_(1e18);
            uint256 _dividends = SafeMath.div(
                SafeMath.mul(_ethereum, entryFee_),
                100
            );
    ...
    function calculateTokensReceived(
        ...
        uint256 _dividends = SafeMath.div(
            SafeMath.mul(_ethereumToSpend, entryFee_),
            100
        );
    ...
    function exitFee() public view returns (uint8) {
        ...
        uint8 currentChange = uint8(
            (totalChange * secondsPassed) / exitFeeFallDuration_
        );
    ...
    function purchaseTokens(
        ...
        uint256 _undividedDividends = SafeMath.div(
            SafeMath.mul(_incomingEthereum, entryFee_),
            100
        );
        uint256 _referralBonus = SafeMath.div(
            SafeMath.mul(_incomingEthereum, refferalFee_),
            100
        );
        ...
        if (
            ...
            referralBalance_[apex] = SafeMath.add(
                referralBalance_[apex],
                (_incomingEthereum * 2) / 100
            );
    ...
    function mint() public returns (bool) {
        ...
        tokenPriceIncremental_ = SafeMath.mul(
            tokenPriceIncremental_,
            prevSupply
        );
        tokenPriceIncremental_ = SafeMath.div(
            tokenPriceIncremental_,
            tokenSupply_
        );
        ...
        if (mintFee <= dividendsOf(msg.sender)) {
            ...
            dividendPaidTotal[msg.sender] = dividendPaidTotal[msg.sender].add(
                mintFee
            );
        }
        //else if user has referral earning available more than mintFee, then deduct from it
            ...
            referralBalance_[msg.sender] = referralBalance_[msg.sender].sub(
                mintFee
            );
        }
        ...
        transferIns[msg.sender].push(
            transferInStruct(
                uint128(tokenBalanceLedger_[msg.sender]),
                uint64(now)
            )
        );
    ...
    function getCoinAge(
        address _address,
        uint _now
    ) internal view returns (uint _coinAge) {
        ...
        for (uint i = 0; i < transferIns[_address].length; i++) {
            ...
            _coinAge = _coinAge.add(
                uint(transferIns[_address][i].amount) *
                    nCoinSeconds.div(2 minutes)
            );
        }
