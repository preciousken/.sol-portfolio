...
contract DiamondDividend is owned {
    ...
    */
    function getDividendPotential() public view returns (uint256) {
        ...
        for (uint i = 0; i < totalGameContracts; i++) {
            uint256 amount = InterfaceGAMES(whitelistCallerArray[i])
                .getAvailableDiamondRake();
            if (amount > 0) {
        ...
        if (totalDividend > 0 || dividendAccumulated > 0) {
            ...
            uint256 newAmount = (totalDividend * divPercentageSUN) /
                100 /
                1000000;
    ...
    function distributeMainDividend()
        public
        onlySigner
        returns (string, uint256, uint256, uint256)
    {
        uint256 frozenDiamondsGlobal = InterfaceDIAMOND(diamondContractAddress)
        ...
        if (msg.sender != signer) {
            require(
                dividendPaidLastTime + 604800 < now,
                "You need to wait 7 days to Do This"
            );
        }
        ...
        for (uint i = 0; i < totalGameContracts; i++) {
            ...
            uint256 amount = InterfaceGAMES(gameAddress)
                .getAvailableDiamondRake();
            if (amount > 0) {
                ...
                require(
                    InterfaceGAMES(gameAddress).requestDiamondRakePayment(),
                    "could not transfer trx"
                );
            }
        }
        ...
        if (totalDividend > 0 || dividendAccumulatedLocal > 0) {
            ...
            if (divPercentageSUN < 100000000) {
                dividendRemainder +=
                    (totalDividend * (100000000 - divPercentageSUN)) /
                    100 /
                    1000000;
            }
            ...
            return (
                "Distro Success: (1) Total Fetched (2) Accumulated Dividend (3) Final Payable amount:",
                totalDividend,
                dividendAccumulatedLocal,
                payableAmount
            );
        }
    }
     ...
     */
    function reInvestDividendRemainder()
        public
        payable
        onlyOwner
        returns (string)
    ...
    */
    function userConfirmedDividendTRX(
        address user
        ...
        uint256 previousDivAmountWithdrawalLocal = previousDivAmountWithdrawal[
            user
        ];
        uint256 usersDiamondFrozen = InterfaceDIAMOND(diamondContractAddress)
            .usersDiamondFrozen(user);
        ...
        if (
            ...
            uint256 newDividendPoolAmount = divPaidAllTimeTrx -
                previousDivAmountWithdrawalLocal;
            uint256 totalFrozenShares = diamondsFrozenAtDivDistribution -
                totalFrozenDiamondTracker[user];
            uint256 totalNoOfDivPaid = totalDividendsPaidNumber -
                noOfDivPaidAfterFreeze[user];
            ...
            uint256 sharePercentage = (usersDiamondFrozen * 100 * 1000000) /
                (totalFrozenShares / totalNoOfDivPaid);
            ...
            if (newDividendPoolAmount * sharePercentage > 0) {
                return
                    (newDividendPoolAmount * sharePercentage) / 100 / 1000000;
            }
        }
    ...
    */
    function withdrawDividendsEverythingInternal(
        address user
    ...
    */
    function withdrawDividendsEverything() public returns (bool) {
        ...
        require(
            withdrawDividendsEverythingInternal(user),
            "withdraw dividend everything function did not work"
        );
     ...
     */
    function addWhitelistGameAddress(
        address _newAddress
        ...
        whitelistCallerArrayIndex[_newAddress] =
     ...
     */
    function removeWhitelistGameAddress(
        address _address
    ) public onlyOwner returns (string) {
        ...
        address lastElement = whitelistCallerArray[
            whitelistCallerArray.length - 1
        ];
     ...
     */
    function changeGlobalHalt() public onlyOwner returns (string) {
        if (globalHalt == false) {
            globalHalt = true;
        } else {
            globalHalt = false;
